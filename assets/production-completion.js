(()=>{
  ...
  function enhance(){
    ...
    const map=planMap();table.querySelectorAll('tbody tr').forEach(row=>{if(row.querySelector('[data-prod-cell]'))return;const planInput=row.querySelector('[data-plan]');if(!planInput)return;const id=planInput.dataset.plan,plan=map[id];const td=document.createElement('td');td.dataset.prodCell='1';
      ...
      const wrap=document.createElement('div');wrap.style.cssText='display:flex;gap:6px;min-width:190px';const qty=document.createElement('input');qty.dataset.prodQty=id;qty.type='number';qty.min='0.001';qty.step='0.001';qty.value=String(remaining>0?remaining:1);qty.style.minWidth='88px';const btn=document.createElement('button');btn.className='btn';btn.dataset.prodSave=id;btn.type='button';btn.disabled=disabled;btn.textContent=plan.status==='completed'?'Concluído':plan.status==='cancelled'?'Cancelado':'Registrar';wrap.append(qty,btn);td.appendChild(wrap);row.appendChild(td);if(!disabled)btn.onclick=()=>record(plan,qty,btn);
    })
  }
  async function record(plan,input,btn){const quantity=Number(input?.value||0);if(!Number.isFinite(quantity)||quantity<=0)return toast('Informe uma quantidade válida.');btn.disabled=true;input.disabled=true;btn.textContent='Registrando…';if(!btn.dataset.requestId)btn.dataset.requestId=crypto.randomUUID();const requestId=btn.dataset.requestId;const {error}=await sb.rpc('padoka_record_production',{p_plan_id:plan.id,p_quantity:quantity,p_request_id:requestId});if(error){btn.disabled=false;input.disabled=false;btn.textContent='Tentar novamente';toast(friendly(error));return}delete btn.dataset.requestId;toast('Produção registrada e estoque atualizado.');await refresh()}
