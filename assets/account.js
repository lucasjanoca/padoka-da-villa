...
$('markNotificationsRead').onclick=async()=>{const epoch=lifecycleEpoch,userId=activeUserId;if(!isCurrent(epoch,userId))return;try{const {error}=await sb.from('padoka_customer_notifications').update({read_at:new Date().toISOString()}).is('read_at',null);if(!isCurrent(epoch,userId))return;if(error)throw error;await loadAccountExtras(epoch,userId);if(isCurrent(epoch,userId))toast('Notificações marcadas como lidas.')}catch(e){if(isCurrent(epoch,userId)){console.error(e);toast('Não foi possível atualizar as notificações.')}}};
