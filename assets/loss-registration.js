(()=>{
  ...
  async function submit(){
    const btn=$('lossSave');if(!btn)return;
  ...
  function intercept(e){const btn=e.target.closest?.('#lossSave');if(!enabled||!btn)return;e.preventDefault();e.stopImmediatePropagation();submit()}
  async function start(){
    for(let n=0;n<100&&!window.padokaSupabase;n++)await new Promise(r=>setTimeout(r,100));sb=window.padokaSupabase;if(!sb)return;
    for(let n=0;n<100&&$('app')?.classList.contains('hidden');n++)await new Promise(r=>setTimeout(r,100));if($('app')?.classList.contains('hidden'))return;
    const probe=await sb.from('padoka_losses').select('request_id').limit(1);
    if(probe.error){if(!missing(probe.error))console.error('PADOKA loss capability:',probe.error);return}
    enabled=true;restorePending();document.addEventListener('click',intercept,true);if(pending)setTimeout(applyPending,120)
  }
  start();
})();