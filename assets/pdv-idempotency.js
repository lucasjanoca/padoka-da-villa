...
function controls(lock){['scanner','scanBtn','search','clear','paymentMethod'].forEach(id=>{const el=get(id);if(el)el.disabled=lock});document.querySelectorAll('.products button,.qty button').forEach(b=>b.disabled=lock)}
...
function update(){if(baseUpdate)baseUpdate();if(!enabled)return;const btn=get('finishSale');if(!btn)return;const lock=!!pending;controls(lock);if(lock){btn.disabled=false;btn.classList.add('ready');btn.textContent='Tentar novamente';statePending()}}
...
function loadScannerFix(){
  try{
    ...
    s.src='assets/pdv-scanner-fix.js?v=2026082601';
