...
function fillSettings(s){$('setEnabled').checked=!!s.enabled;$('setRate').value=s.points_per_brl??1;$('setFirst').value=s.first_order_bonus_points??20;$('setBirthday').value=s.birthday_multiplier??2;$('setDays').value=s.redemption_valid_days??30;$('setCap').value=s.max_points_per_order??5000}
async function saveSettings(){
  try{const {error}=await sb.rpc('padoka_admin_update_loyalty_settings',{p_enabled:$('setEnabled').checked,p_points_per_brl:Number($('setRate').value),p_first_order_bonus_points:Number($('setFirst').value),p_birthday_multiplier:Number($('setBirthday').value),p_redemption_valid_days:Number($('setDays').value),p_max_points_per_order:Number($('setCap').value)});if(error)throw error;notice('settingsNotice','Configurações salvas.','ok');await loadManager()}
