...
ok(source.indexOf('if(INTERNAL_PAGES.has(currentPage))return;') < source.indexOf("const ENDPOINT='https://yncspxfsvlqdnodlsosb.supabase.co/functions/v1/padoka-telemetry'"), 'telemetry: bloqueio interno deve ocorrer antes de configurar o endpoint');
ok(source.indexOf('if(INTERNAL_PAGES.has(currentPage))return;') < source.indexOf("sessionStorage.getItem(KEY)"), 'telemetry: página interna não deve criar/restaurar sessão de telemetria');
