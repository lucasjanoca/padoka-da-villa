...
need(/async function markOneRead\(id,epoch=lifecycleEpoch,userId=activeUserId\)[\s\S]*?if\(!await sessionStillCurrent\(epoch,userId\)\)return false;[\s\S]*?client\.rpc\(['"]padoka_mark_customer_notifications_read['"],[\s\S]*?p_notification_id:id[\s\S]*?if\(!await sessionStillCurrent\(epoch,userId\)\)return false;/i,'marcação individual precisa fazer preflight e recheck da identidade ao redor da RPC');
need(/async function markAllRead\(\)[\s\S]*?const epoch=lifecycleEpoch;\s*const userId=activeUserId;[\s\S]*?if\(!await sessionStillCurrent\(epoch,userId\)\)return false;[\s\S]*?client\.rpc\(['"]padoka_mark_customer_notifications_read['"],[\s\S]*?p_notification_id:null[\s\S]*?if\(!await sessionStillCurrent\(epoch,userId\)\)return false;/i,'marcação em lote precisa capturar a identidade e fazer preflight/recheck ao redor da RPC');
need(/\.eq\(['"]user_id['"],userId\)/i,'leituras precisam continuar vinculadas explicitamente ao user_id atual');
