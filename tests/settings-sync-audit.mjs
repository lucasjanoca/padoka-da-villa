...
const checks=[
  ...
  [frontend.includes("const session=await safeSession('role check')"), 'role polling must fail closed when the current session cannot be confirmed'],
  [frontend.includes("const session=await safeSession('activation')"), 'activation must re-confirm the same session after role validation'],
  [frontend.includes("const session=await safeSession('startup')"), 'startup must keep settings blocked when session confirmation fails'],
  [frontend.includes("blockLegacyFallback('Confirmando sessão e permissões internas…')"), 'settings must start blocked before the first session check'],
