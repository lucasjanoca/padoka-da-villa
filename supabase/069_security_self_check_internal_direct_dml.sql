-- PADOKA DA VILLA — amplia o self-check para impedir regressão de escrita direta interna
...
begin
  ...
  select coalesce(jsonb_agg(c.relname order by c.relname),'[]'::jsonb)
    into v_bad_direct_dml
  from pg_catalog.pg_class c
  join pg_catalog.pg_namespace n on n.oid=c.relnamespace
  where n.nspname='public'
    and c.relkind='r'
    and c.relname = any(array[
      'padoka_inventory','padoka_inventory_movements','padoka_production_plans','padoka_production_batches',
      'padoka_losses','padoka_sales','padoka_sale_items','padoka_settings','padoka_suppliers',
      'padoka_purchase_orders','padoka_purchase_order_items','padoka_coupons','padoka_incidents',
      'padoka_staff_audit','padoka_product_audit','padoka_payment_attempts','padoka_payment_events',
      'padoka_payment_settings','padoka_push_config','padoka_push_subscriptions'
    ]::text[])
    and (
      ...
      or pg_catalog.has_any_column_privilege('authenticated',c.oid,'INSERT')
      or pg_catalog.has_any_column_privilege('authenticated',c.oid,'UPDATE')
    );

  if jsonb_array_length(v_bad_direct_dml)>0 then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','internal_direct_dml_restored',
      'tables',v_bad_direct_dml
    ));
  end if;
