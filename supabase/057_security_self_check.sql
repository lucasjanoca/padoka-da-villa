...
begin
  ...
  if jsonb_array_length(v_bad_functions)>0 then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','public_security_definer_exposure',
      'functions',v_bad_functions
    ));
  end if;
  ...
  if exists(
    ...
    where table_schema='public'
      and table_name='padoka_push_config'
      and column_name='vapid_private_key'
  ) then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','vapid_private_key_public_schema'
    ));
  end if;
  ...
  if pg_catalog.has_table_privilege('anon','public.padoka_push_subscriptions','SELECT')
    ...
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','push_subscriptions_client_read'
    ));
  end if;
  ...
  if pg_catalog.has_function_privilege('anon','public.padoka_get_push_server_config()','EXECUTE')
    ...
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','push_server_secret_rpc_exposed'
    ));
  end if;

  if pg_catalog.has_schema_privilege('anon','padoka_rpc_private','USAGE') then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','private_rpc_schema_anon_usage'
    ));
  end if;
  ...
  if pg_catalog.has_function_privilege('anon','public.padoka_admin_resolve_incident(uuid)','EXECUTE')
    ...
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','enterprise_admin_rpc_anon_execute'
    ));
  end if;
  ...
  if pg_catalog.has_column_privilege('authenticated','public.padoka_feature_flags','enabled','UPDATE')
    ...
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'check','sensitive_direct_update_restored'
    ));
  end if;
  ...
  if jsonb_array_length(v_issues)>0 then
    ...
    values(
      ...
      'PADOKA security self-check failed',1,
...
begin
  for v_jobid in
    select jobid from cron.job where jobname='padoka-security-self-check-hourly'
  loop
...
select cron.schedule(
  'padoka-security-self-check-hourly',
  '7 * * * *',
  'select padoka_private.padoka_security_self_check();'
);
